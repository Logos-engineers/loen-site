...
const BottomNav = ({ current, ink, mute, accent, show }) => {
  if (!show) return null;
  const items = [
    { l:"수련회 정보", k:"수련회 정보" },
    { l:"굿즈", k:"굿즈" },
  ...
  return (
    <div style={{
      ...
      background:"var(--bg)", borderTop:`1px solid ${mute}`,
      display:"flex", alignItems:"stretch",
    }}>
      {items.map(it => (
        <div key={it.k} style={{
          flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:4,
          color: it.k===current ? ink : ink, opacity: it.k===current ? 1 : .42,
        }}>
          <div style={{
            width:14, height:14, borderRadius:3,
            border:`1.5px solid ${it.k===current?accent:ink}`,
            background: it.k===current ? accent : "transparent",
          }}/>
          <div style={{fontSize:9.5, fontWeight: it.k===current?600:400, letterSpacing:".02em"}}>{it.l}</div>
        </div>
      ))}
    </div>
  );
